...
class GraphExporter():
    ...
    def __init__(self, db_name, config, log_root_path):
        ...
        self._log_path = log_root_path + "export-graph-" + db_name
    ...
    def _load_graph_exporter_json(self, json_path):
        ...
        settings = data.get('graph')

        if not settings:
            self._logger.error("GraphExporter: the settings file cannot be loaded!")
    ...
    def export(self, file_path, settings_path):
        ...
        try:
            ...
            settings = self._load_graph_exporter_json(settings_path)

            if settings:
                metric_name = settings.get("name")
