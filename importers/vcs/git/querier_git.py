...
class GitQuerier():
    ...
    def get_file_content(self, state, file):
        """
        gets content of a file for a given state (reference or commit)

        :type state: str
        :param state: name of the reference or commit SHA
        ...
        try:
            git = self._repo.git
            found = git.execute(["git", "show", state + ":" + file])
        except:
            found = None
        return found
