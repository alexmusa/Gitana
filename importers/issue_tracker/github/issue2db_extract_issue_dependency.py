    ...
    def _extract_issue_dependencies(self):
        ...
        while row:
            try:
                ...
                comments = [(self._querier.get_issue_body(issue), self._querier.get_issue_creation_time(issue))] + \
                           [(self._querier.get_issue_comment_body(comment), self._querier.get_issue_comment_creation_time(comment)) for comment in self._querier.get_issue_comments(issue)]

                for c, created_at in comments:
                    if c:
                        ...
                        for ri in referenced_issues:
                            ...
                            self._dao.insert_issue_dependency(issue_id, referenced_issue_id, created_at, self._dao.get_issue_dependency_type_id("related"))
